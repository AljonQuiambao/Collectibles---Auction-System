<?php
	...
	if (isset($_GET['uid'])) {
		if (!isset($_GET['convo_id'])) {
			...
			$chk = $link->query("SELECT * from thread where $cdata or $cdata2 ");
			if ($chk->num_rows > 0) {
		...
		$user = $link->query("SELECT * FROM users where id =" . $_GET['uid'])->fetch_array();
	} else {
		$thread = $link->query("SELECT * from thread where md5(id) = '" . $_GET['convo_id'] . "' ")->fetch_array();
		$user = $link->query("SELECT * FROM users where id in ({$thread['user_ids']}) and id !=" . $_SESSION['login_id'])->fetch_array();
	}
?>
<div class="w-100" id="cheader">
	<table class="" width="100%">
		<tr>
			...
			<td width="3%"><a href="index.php?page=home" title="Back"><i class="fa fa-arrow-left"></i></a></td>
			<td width="3%"><a href="<?php $_SERVER['PHP_SELF']; ?>" title="Refresh"><i class="fas fa-sync"></i></a></td>
		</tr>
	</table>
</div>
		...
		<tr>
			<td width="90%">
				<form id="send_chat">
					...
					<textarea class="form-control w-100" name="message" placeholder="Type Message ..."></textarea>
				</form>

			</td>
			<td width="10%">
				<button class="btn btn-primary mt-3" type="button" onclick="$('#send_chat').submit()"> Send 
...
<script>
	...
	$('#send_chat').submit(function(e) {
		...
		var data = {
			name: '<?php echo $_SESSION['login_name'] ?>',
			id: '<?php echo $_SESSION['login_id'] ?>',
			avatar: '<?php echo $_SESSION['login_avatar'] ?>',
			...
			success: function(resp) {
				...
				if (resp.status == 1) {
					...
					websocket.send(JSON.stringify({
						type: 'chat_sent',
						data: data,
						convo_id: resp.convo_id,
						by: '<?php echo $_SESSION['login_id'] ?>',
						user_ids: resp.convo_users
					}))
				}
			}
		})
	...
	window.chat_box = function(data, count, loaded = 0) {
		...
		$.ajax({
			...
			method: 'POST',
			data: {
				convo_id: "<?php echo $convo_id ?>",
				user_id: '<?php echo isset($user) ? $user['id'] : '' ?>'
			},
			success: function(resp) {
				if (resp == 1) {
			...
			success: function(resp) {
				if (resp) {
					...
					if (Object.keys(resp).length > 0) {
						...
						$('#cfield').html("<center>No Message Available</center>")
					}
				}
			},
			complete: function() {
